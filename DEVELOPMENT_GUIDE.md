...
### 7. 평가항목 자동 매칭(유사 질문 후보 제시 + 사용자 확인)

Excel/CSV 열 제목이 등록된 질문과 정확히 같지 않아도, 문자 bigram 유사도로 가장 가까운 후보를 제시하고
**항상 사용자가 화면에서 확인·수정**하게 합니다(자동 저장하지 않음). 새로운 "업로드 → 매칭 확인 → 반영"
플로우가 필요하면 이 엔진을 재사용하세요(최종평가 결과 입력의 "응답 Excel 파일" 입력 방식이 대표 사례).

| 함수 | 역할 |
|---|---|
| `diceSimilarity(a, b)` / `textBigrams(s)` | 문자 bigram 기반 유사도(0~1). AI가 아닌 순수 문자열 계산이며, 항상 재현 가능하다. |
| `bestTemplateMatch(headerText, rows, usedIndices)` | 헤더 1개에 대해 가장 유사한 템플릿 행을 찾는다. 정확히 일치하면 `status:'matched'`, 유사도 ≥0.35면 `status:'review'`(확인 필요), 그 외 `status:'unmatched'`. |
| `autoMatchHeaders(rows, headers)` | 헤더 배열 전체를 순서대로 그리디 매칭(같은 템플릿 행이 두 열에 중복 배정되지 않도록 `usedIndices`로 추적). |
| `finalMatchStatusBadge(status)` | 매칭 상태 배지(매칭 완료/확인 필요/매칭 안됨) — 매칭 결과 표에서 공용으로 사용. |
| `averageColumnWithRangeCheck(rows, colIndex, scale)` | 점수형 열 평균 + 척도 범위를 벗어난 응답 건수(`outOfRangeCount`)를 함께 반환 — 사용자에게 확인 안내를 표시할 때 사용. |
| `choiceColumnRates(rows, colIndex, options)` | 선택형 열의 옵션별 응답 비율(%). 옵션 문자열이 서로의 부분 문자열인 경우(예: "추천"/"비추천") **정확히 일치하는 옵션을 우선**하도록 처리되어 있다 — 새로운 옵션 매칭 로직을 만들 때도 이 함정을 주의할 것. |
